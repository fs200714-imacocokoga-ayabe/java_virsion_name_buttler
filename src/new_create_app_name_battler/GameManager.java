...
public class GameManager {
			...
			for (int i = 0; i < attackList.size(); i++) {// attackに格納したplayerが全員行動する
				...
				if (player1.isLive()) {
					if (player1.isMark()) {// player1が敵の場合
						...
						strategyData = selectStrategyNumber(allyStrategyNumber);
				...
				player2 = party.selectMember(strategyData[0]);// 作戦で選んだ相手を呼ぶ
				player1.attack(player2, strategyData[1]);// player1に相手と作戦を送り攻撃する
	...
	private int[] selectStrategyNumber(int number) {
		...
		case 3:
			...
			break;
		case 4:
			context = new Context(new Strategy4());
			break;
		case 5:
			context = new Context(new Strategy5());
			break;
		}

		strategyData = context.attackStrategy(player1, party.getParty1(),
				party.getParty2());
	...
	private void fixePlayer() throws InterruptedException {
		...
		jobList.add(new Fighter("アシュラム"));
		jobList.add(new Wizard("スレイン"));
		jobList.add(new Priest("ゼロス"));
		jobList.add(new Ninja("ウッド"));
		jobList.add(new Wizard("ガンダルフ"));
		jobList.add(new Priest("バグナード"));
	...
	private void makePlayer() {
		...
		jobList.add(new Fighter(""));// プレイヤー作成のメニュに表示するため初期設定jobList0に格納
		jobList.add(new Wizard(""));// 初期設定jobList1に格納
		jobList.add(new Priest(""));// 初期設定jobList2に格納
		jobList.add(new Ninja(""));// 初期設定jobList3に格納
	...
	private void JobSelect(int job, String name) {
		...
		case 1:
			jobList.add(new Fighter(name));
			break;

		case 2:
			jobList.add(new Wizard(name));
			break;

		case 3:
			jobList.add(new Priest(name));
			break;

		case 4:
			jobList.add(new Ninja(name));
			break;
